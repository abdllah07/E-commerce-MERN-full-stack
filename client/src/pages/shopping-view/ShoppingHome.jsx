...
import bannerOne from '../../assets/banner-1.webp'; // Import banner images for the slider
...
function ShoppingHome() {
    ...
    const slides = [bannerOne, bannerTwo, bannerThree]; // Array of banner images for the slideshow
   ...
   // Open the product details dialog when product details are fetched
    ...
    useEffect(() => {
        const timer = setInterval(() => {
            setCurrentSlide(prev => (prev + 1) % slides.length); // Change the slide every 5 seconds
        }, 5000);
                ...
                <div className='container mx-auto px-4'>
                    ...
                    <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4'>
                        {categoriesWithIcons.map(category => (
                            <Card onClick= {() => handleNavigateToListingPage(category , 'category')} key={category.id} className='border-l-[#001F3F] cursor-pointer hover:shadow-slate-600 hover:-translate-y-3 transition-all bg-[#F5F5F5] hover:bg-[#F5F7F8]'>
